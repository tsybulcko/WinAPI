...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		HWND hButtonZero = CreateWindowEx      //Создаем кнопку
		(
			NULL, "Button", "0",
			WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_BITMAP,
		...
		HBITMAP hbmpZero = (HBITMAP)LoadImage  //Создаем картинку
		(
			GetModuleHandle(NULL),
			"Zero.bmp",
			IMAGE_BITMAP,
			LR_DEFAULTSIZE, LR_DEFAULTSIZE,
			//g_i_DOUBLE_BUTTON_SIZE, g_i_BUTTON_SIZE,      //Рстянуть рисунок на всю кнопку
			LR_LOADFROMFILE
		);
		SendMessage(hButtonZero, BM_SETIMAGE, 0, (LPARAM)hbmpZero); //Натягиваем картинку на кнопку
	...
	break;
	case WM_CTLCOLOREDIT:   // Меняем цвет, любые действия с цветом выполняются через HDC - девайсконтрол
	{
	...
	case WM_COMMAND:
	{
		...
		static INT operation = 0;               // Операции
		static BOOL input = FALSE;              // Срабатывает при нажатии цифр
		static BOOL input_operation = FALSE;    // Срабатывает при нажатии на знак орерации
		...
		if (LOWORD(wParam) >= IDC_BUTTON_0 && LOWORD(wParam) <= IDC_BUTTON_9)
		{
			if (input_operation)
			{
				...
				sz_buffer[0] = 0;     //Заклиниваем буффер
		...
		if (LOWORD(wParam) == IDC_BUTTON_EQUAL)
		{
			...
			input_operation = FALSE;     //Сбрасываем операцию
			sprintf(sz_buffer, "%g", a); // Загоняем результат в строку
			SendMessage(hEdit, WM_SETTEXT, 0, (LPARAM)sz_buffer); //Выводим результат на экран
		}
	}
	break;
	case WM_KEYDOWN:
...
VOID SetSkin(HWND hwnd, CONST CHAR skin[])
{
	...
	for (int i = 0; i < 10; i++)
	{
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%i.bmp", skin, i);  //Собираем путь к файлику
		HWND hButton = GetDlgItem(hwnd, IDC_BUTTON_0 + i);    //Получаем дескриптор кнопки
...
VOID SetSkinFromDLL(HWND hwnd, CONST CHAR skin[])
{
	HMODULE hSkin = LoadLibrary(skin); //Загружаем DLL в память
	for (INT i = IDC_BUTTON_0; i <= IDC_BUTTON_EQUAL; i++)
	{
		...
		(
			...
			MAKEINTRESOURCE(i),
			IMAGE_BITMAP,
			i == IDC_BUTTON_0 ? g_i_DOUBLE_BUTTON_SIZE : g_i_BUTTON_SIZE,     // Координата по Х
			i == IDC_BUTTON_EQUAL ? g_i_DOUBLE_BUTTON_SIZE : g_i_BUTTON_SIZE, //Координата по Y
			LR_SHARED  //означает загрузку из разделяемой библиотеки
		);
		SendMessage(GetDlgItem(hwnd, i), BM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBitmap);//Натягиваем картинку на кнопку
	}
	FreeLibrary(hSkin); //Эта функция вызываетя вместе с LoadLibrary в обязательном порядке
}